...
Stack.prototype.addPiece = function(tetro) {
  ...
  if (piecesSet == 500) {
    const url = '../shoppers-gh-pages/test3.php';
  
    // Make a POST request to test3.php
    fetch(url, {
      method: 'POST',
      headers: {
      ...
      // You can include a body if your server expects POST data
      // body: JSON.stringify({ key: 'value' }),
    })
    .then(response => response.text())
    .then(data => {
      // Handle the response from the server
      console.log(data); // You might want to do something more meaningful here
    })
    .catch(error => {
