<?php
   ...
   $category = isset($_SESSION["cID"]) ? $_SESSION["cID"] : "";


   if(isset($_POST["edit"]))
   {
      $_SESSION["cID"]=$_POST["cID"];
      header("location:admin_show_category_edit.php");
   }
...
if(isset($_GET['page']) && is_numeric($_GET['page'])){
    $_SESSION['current_pageCategory'] = $_GET['page'];
    $page = $_GET['page'];
} else if(isset($_SESSION['current_pageCategory']) && is_numeric($_SESSION['current_pageCategory'])) {
    $page = $_SESSION['current_pageCategory'];
} else {
    $page = 1;
}
                        ...
                        <div class="col-md-12">
                           <div class="page_title">
                              <div class="search">
                                 ...
                                 <form action="admin_show_category.php" method="POST">
                                       <select name="filter"> <!--Filter the result-->
                                          <option value="cID">Category ID </option>
                                          <option value="cName">Category Name</option>                      
                                       </select>
                              ...
                              <table class="table table-striped">
                                  <thead>
                                    <tr>       
                                       ...
                                       <tbody>
                                       <?php 
                                                $sql = "SELECT * FROM category LIMIT $offset, $records_per_page";
                                                ...
                                                {                                                    
                                                    ...
                                                    if ($filter == "cID") 
                                                    {
                                                        ...
                                                        $sql = "SELECT * FROM category WHERE cID = '$title' LIMIT $offset, $records_per_page";
                                                            if ($title == "") 
                                                            {
                                                                $sql = "SELECT * FROM category LIMIT $offset, $records_per_page";
                                                            }
                                                    ...
                                                    else if ($filter == "cName") 
                                                    {
                                                        ...
                                                        // Add appropriate conditions to the query based on the filter and key values
                                                        $sql = "SELECT * FROM category WHERE cName LIKE '%$title%' LIMIT $offset, $records_per_page";
                                                            if ($title == "") 
                                                            {
                                                                $sql = "SELECT * FROM category LIMIT $offset, $records_per_page";
                                                            }
                                                    ...
                                                    else if ($filter == "Image") 
                                                    {
                                                        ...
                                                        // Add appropriate conditions to the query based on the filter and key values
                                                        $sql = "SELECT * FROM category WHERE *Image LIKE '%$title%' LIMIT $offset, $records_per_page";
                                                            if ($title == "") 
                                                            {
                                                                $sql = "SELECT * FROM category LIMIT $offset, $records_per_page";
                                                            }
                                       ...
                                       <tr>
                                          <form action="admin_show_category.php" method="post">                                       
                                             ...
                                             <td> <img class="img-responsive rounded-circle" src="../Admin/image/<?php echo $category["Image"]?>" style="width:25%;"></td>                                               
                                             <input type="hidden" value="<?php echo $category["cID"]?>" name="cID">      
