...
$image = "images.jpg"; // Retain the existing image if no file was uploaded
...
if (isset($_POST["submit"])) {
   ...
   $Image = isset($_POST["Image"]) ? $_POST["Image"] : "";
    ...
    $pwErr = $verification->Errorpw($Password);

   if (empty($nameErr) && empty($mailErr) && empty($pwErr)) {
      // Check if a file was uploaded
      if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
          $FileUploader = new FileUploader($_FILES);
          $Image = $uploadMessage = $FileUploader->uploadProductPicUser();
          // Update the user data in the database
          $Login->checkUserAdmin($Name, $Password, $Email,$Image);
      } 
      else 
      {
