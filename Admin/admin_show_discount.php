<?php
   ...
   if(isset($_POST["edit"]))
   {
      ...
      header("location:admin_show_discount_edit.php");
   }
...
if(isset($_GET['page']) && is_numeric($_GET['page'])){
    $_SESSION['current_pageDiscount'] = $_GET['page'];
    $page = $_GET['page'];
} else if(isset($_SESSION['current_pageDiscount']) && is_numeric($_SESSION['current_pageDiscount'])) {
    $page = $_SESSION['current_pageDiscount'];
} else {
    $page = 1;
}
...
$total_query = "SELECT COUNT(*) as total FROM discount";
                        ...
                        <div class="col-md-12">
                           <div class="page_title">
                              <div class="search">
                                 ...
                                 <form action="admin_show_discount.php" method="POST">
                                       <select name="filter"> <!--Filter the result-->
                                          <option value="Oid">Discount ID </option>
                                          <option value="offer">Offer</option>
                                          <option value="percentage">Percentage (in decimal)</option>
                                          <option value="userID">userID</option>
                                       </select>
                                       <input type="text" placeholder="Search" name="title">
                                       <button type="submit" name="search" class="search-button">Search</button>
                                       <button class="search-button" style="float: right; margin-right: 15px;" name="add"><a href="addDiscount.php" style="color: white;">Add Discount</a></button> 
                                    </form>                
                                 </div>
                              </div>
                           </div>
                              ...
                              <table class="table table-striped">
                                  <thead>
                                    <tr>       
                                       ...
                                       <tbody>
                                       <?php 
                                                $sql = "SELECT * FROM discount LIMIT $offset, $records_per_page";
                                                ...
                                                {                                                    
                                                    ...
                                                    if ($filter == "Oid") 
                                                    {
                                                        ...
                                                        $sql = "SELECT * FROM discount WHERE Oid = '$title' LIMIT $offset, $records_per_page";
                                                            if ($title == "") 
                                                            {
                                                                $sql = "SELECT * FROM discount LIMIT $offset, $records_per_page";
                                                            }
                                                    ...
                                                    {
                                                        // Add appropriate conditions to the query based on the filter and key values
                                                        $sql = "SELECT * FROM discount WHERE offer LIKE '%$title%' LIMIT $offset, $records_per_page";
                                                            if ($title == "") 
                                                            {
                                                                $sql = "SELECT * FROM discount LIMIT $offset, $records_per_page";
                                                            }
                                                    ...
                                                    else if ($filter == "percentage") 
                                                    {
                                                        ...
                                                        // Add appropriate conditions to the query based on the filter and key values
                                                        $sql = "SELECT * FROM discount WHERE percentage LIKE '%$title%' LIMIT $offset, $records_per_page";
                                                            if ($title == "") 
                                                            {
                                                                $sql = "SELECT * FROM discount LIMIT $offset, $records_per_page";
                                                            }
                                                    ...
                                                    else if ($filter == "userID") 
                                                    {
                                                        ...
                                                        // Add appropriate conditions to the query based on the filter and key values
                                                        $sql = "SELECT * FROM discount WHERE userID LIKE '%$title%' LIMIT $offset, $records_per_page";
                                                            if ($title == "") 
                                                            {
                                                                $sql = "SELECT * FROM discount LIMIT $offset, $records_per_page";
                                                            }
                                       ...
                                       <tr>
                                          <form action="admin_show_discount.php" method="post">                                       
                                             <td><?php echo $discount["Oid"]?></td>
                                             <td><?php echo $discount["offer"]?></td>
                                             <td><?php echo $discount["percentage"]?></td>
                                             <td><?php echo $discount["userID"]?></td> 
                                             <td><?php echo $discount["available"]?></td>                                                   
                                             <td><?php echo $discount["date"]?></td>                                                   
                                                  
                                             <input type="hidden" value="<?php echo $discount["Oid"]?>" name="Oid">      
