...
$product=$Login->getProductDetail($pID);
...
if (isset($_POST["submit"])) {
   ...
   $pNameErr = $verification->pName($pName);
   $priceErr = $verification->price($price);
   $seriesErr = $verification->ErrorName($series);
   $sizeErr = $verification->size($size);
   $colorErr = $verification->color($color);
   $qtyErr = $verification->qty($qty);
      // Check if a file was uploaded
   if(empty($pNameErr) AND empty($priceErr) AND empty($seriesErr)AND empty($sizeErr)AND empty($colorErr)AND empty($qtyErr))
   {   
      if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
          $FileUploader = new FileUploader($_FILES);
          $image = $uploadMessage = $FileUploader->uploadProductPicAdmin();
          // Update the user data in the database
          $Login->updateProduct($pID,$pName,$series,$image,$size,$price,$details,$qty,$status,$color,$category,$video);
      } 
      else 
      {
          $image = $product["image"]; // Retain the existing image if no file was uploaded
          // Update the user data in the database
          $Login->updateProduct($pID,$pName,$series,$image,$size,$price,$details,$qty,$status,$color,$category,$video);
      }
                           ...
                           <div class="white_shd full margin_bottom_30">
                              ...
                              <div class="full price_table padding_infor_info">
                                 <div class="row">
                                    ...
                                    <!-- profile image -->
                                    <form action="admin_show_product_edit.php" method="post" enctype="multipart/form-data">
                                    <div class="col-lg-12">
                                       <div class="full dis_flex center_text">
                                          <div class="profile_img">
                                             <img width="180" class="rounded-circle" src="../Admin/image/<?php echo $product["image"]?>" alt="#" /><br>
                                             <!-- File upload button -->
                                        ...
                                        <div class="profile_contant">
                                            <div class="contact_inner">
                                             ...
                                             <input type="text" name="series" value="<?php echo $product["series"]?>"><br>
                                             <?php
                                             if ($seriesErr == "") {
                                                // If the error is empty, this row does not appear
                                             ...
                                             <input type="text" name="size" value="<?php echo $product["size"]?>" required><br><br>
                                             <br><label for="video">Video</label><br>
                                             <input type="text" name="video" value="<?php echo $product["video"]?>" required ><br><br>
                                             <label for="color">Color</label><br>
                                             <input type="text" name="color" value="<?php echo $product["color"]?>" required><br>
                                             <br><label for="size">Quantity</label><br>
                                             <input type="number" name="qty" min="1" max="50" value="<?php echo $product["qty"]?>" required><br>                                      
                                             <br><label for="size">Details</label><br>
                                             <textarea name="details" rows="15"cols="40" value="<?php echo $product["details"]?>" required></textarea><br><br>  
